
<img width="148" height="148" alt="image" src="https://github.com/user-attachments/assets/0ad97c2e-ee17-49a2-84a6-35837771d9e3" />


INTELIGENCIA ARTIFICIAL

TALLER DE LIBRERÍA PANDAS

CORTE II

Integrantes:

Kelly Jhoana Mosquera Urbano
Anyela Katerine Rentería Cuama

Programa: Ingeniería de Sistemas

Universidad del Pacífico

2026



# 📊 Taller de Pandas – Análisis de Datos en Python

## 📌 Descripción del proyecto

Este taller tiene como objetivo aplicar los conocimientos adquiridos en la librería **Pandas** de Python para el análisis, manipulación y transformación de datos.

A través de diferentes ejercicios, se trabaja la limpieza de datos, filtrado, ordenamiento y análisis básico de información, con el fin de fortalecer habilidades en el manejo de datos estructurados.

---

## 🎯 Objetivo general

Desarrollar habilidades en el uso de la librería Pandas para la gestión y análisis de datos mediante la implementación de operaciones básicas y avanzadas sobre conjuntos de datos.

---

## 🧰 Herramientas utilizadas

- Python 🐍
- Google Colab
- Pandas 📊
- GitHub

---

## 📁 Contenido del taller

En este taller se desarrollan los siguientes temas:

- Carga y exploración de datos
- Limpieza y tratamiento de datos
- Filtrado de información
- Ordenamiento de datos
- Creación de nuevas columnas
- Análisis básico de datos
- Visualización básica (si aplica)

---

## 📊 Resultados esperados

- Comprensión del manejo de DataFrames en Pandas
- Aplicación de filtros y condiciones
- Organización y limpieza de datos
- Interpretación de resultados obtenidos

---

## 👩‍💻 Autora

**Kelly Mosquera**  
Estudiante de Ingeniería de Sistemas  
Universidad del Pacífico

---

## 📌 Nota

Este proyecto hace parte de un proceso académico de aprendizaje en el uso de herramientas de análisis de datos con Python.
